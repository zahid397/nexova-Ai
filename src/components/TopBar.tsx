...
export function TopBar({ title, onRefresh }: { title: string; onRefresh?: () => void }) {
  ...
  return (
    <div className="sticky top-0 z-20 flex items-center justify-between border-b border-border bg-background/80 px-8 py-4 backdrop-blur-xl">
      <h1 className="text-xl font-bold tracking-tight">{title}</h1>
      <div className="flex items-center gap-2">
        <button className="flex items-center gap-2 rounded-xl bg-card px-3 py-2 text-xs font-medium text-muted-foreground shadow-[var(--shadow-soft)] hover:bg-secondary">
